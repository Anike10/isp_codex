        ...
        foreach (['*1', '*2'] as $id) {
            Http::assertSent(function ($request) use ($id, $router) {
                ...
                return str_contains($onDown, '/tool fetch')
                    ...
                    && str_contains($onDown, ':local webhookUptime "0";')
                    && str_contains($onDown, ':local webhookBytesIn 0;')
                    && str_contains($onDown, ':local webhookBytesOut 0;')
                    && str_contains($onDown, ':local webhookCallerId $"caller-id";')
                    && str_contains($onDown, ':local webhookReason "";')
                    && ! str_contains($onDown, '$"bytes-in"')
                    && ! str_contains($onDown, '$"bytes-out"')
                    && ! str_contains($onDown, '$"last-disconnect-reason"')
                    && ! str_contains($onDown, '$uptime')
