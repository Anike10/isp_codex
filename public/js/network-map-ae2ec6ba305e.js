(function () {
    ...
    function renderPartySearchResults(matches) {
        ...
        const listHtml = uniqueMatches
            ...
            .map((feature) => {
                ...
                return `<button type="button" class="party-id-result" data-focus-customer="${escapeHtml(id)}">#${escapeHtml(id)}</button>`;
            })
            .join('');

        const hasMore = uniqueMatches.length > 60;
        resultsPanel.innerHTML = `<div class="party-id-list">${listHtml}</div>${hasMore ? `<div class="search-empty">Showing first 60 of ${uniqueMatches.length} IDs.</div>` : ''}`;
        resultsPanel.hidden = false;
    ...
    function renderPartyNoLocationPrompt(feature) {
        ...
        resultsPanel.innerHTML = `
            <div class="party-id-list"><span class="party-id-result is-static">#${escapeHtml(customerId)}</span></div>
    ...
    function focusCustomer(customerId, options = {}) {
        ...
        if (!feature || !customerHasLocation(feature)) {
            if (feature) {
                ...
                setCustomerSearchResult(`Party ID #${customerId} has no saved map location.`, true);
            }
            return false;
        }
        ...
        showCustomerPopup(feature);
        setCustomerSearchResult(`Showing Party ID #${customerId}.`);
