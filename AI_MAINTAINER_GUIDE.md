...
- `POST /customers/{customer}/special-price`

Permission:

- `manage_customers`
- `mark_special_customer` for the party-list Special ISP toggle
- `set_special_package_price` for the party-list special package price field
...
- A nullable `subscriptions.custom_price` overrides the current package list
  price for that party. Blank input clears the override. Changing the party's
  package clears the old override because it belongs to the previous package.
- Use `Subscription::effectivePrice()` (or
  `Customer::effectiveMonthlyPrice()`) in billing paths. The effective price
  drives service invoices, renewal affordability, bulk-payment amounts,
  reseller commission, and concession valuation.
...
- When `subscriptions.custom_price` is non-null, all future service bills and
  renewal/payment calculations use it instead of
...
- "Refresh secrets" on those pages runs `refreshActiveRouterSecrets()`
  ...
  `importActiveUsers()`, so currently-connected users that have no `/ppp/secret`
...
- Party creation from imported secrets matches both `connection_id` and
  ...
  a duplicate. A matching soft-deleted party is restored and refreshed because
  its unique identifiers still prevent a new insert.
...
and `onu_power_history.retention_days` (default 7) are edited from the form on
the OLT ONU list (`PATCH olt-onus/power-history-settings`,
