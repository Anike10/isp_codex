...
class PppWebhookService
{
    ...
    public function scriptFor(MikrotikRouter $router): string
    {
        ...
        return ':local webhookUser $user;'
            ...
            .':if ([:len $webhookBytesIn] = 0 || $webhookBytesIn = "null") do={:set webhookBytesIn $"rx-byte";};'
            .':if ([:len $webhookBytesOut] = 0 || $webhookBytesOut = "null") do={:set webhookBytesOut $"tx-byte";};'
            .':if ([:len $webhookBytesIn] = 0 || $webhookBytesIn = "null") do={:set webhookBytesIn $"bytes";};'
            .':if ([:len $webhookBytesOut] = 0 || $webhookBytesOut = "null") do={:set webhookBytesOut $"bytes";};'
            .':local needBytesIn ([:len $webhookBytesIn] = 0 || $webhookBytesIn = "null");'
            .':local needBytesOut ([:len $webhookBytesOut] = 0 || $webhookBytesOut = "null");'
            .':if ($needBytesIn || $needBytesOut) do={'
            .'    :local activeRows [/ppp/active/print as-value stats where name="$webhookUser"];'
            .'    :if ([:len $activeRows] = 0) do={:set activeRows [/ppp/active/print as-value stats where caller-id="$webhookCallerId"]};'
            .'    :foreach row in=$activeRows do={'
            .'        :if ([:len ($row->"bytes-in")] > 0) do={:set webhookBytesIn ($row->"bytes-in");}'
            .'        :if ([:len ($row->"bytes-out")] > 0) do={:set webhookBytesOut ($row->"bytes-out");}'
            .'        :if ([:len ($row->"bytes")] > 0) do={:set webhookBytesOut ($row->"bytes");}'
            .'        :if ([:len ($row->"tx-byte")] > 0) do={:set webhookBytesOut ($row->"tx-byte");}'
            .'        :if ([:len ($row->"rx-byte")] > 0) do={:set webhookBytesIn ($row->"rx-byte");}'
            .'    };'
            .'    :set needBytesIn ([:len $webhookBytesIn] = 0 || $webhookBytesIn = "null");'
            .'    :set needBytesOut ([:len $webhookBytesOut] = 0 || $webhookBytesOut = "null");'
            .'}'
            .':if ($needBytesIn || $needBytesOut) do={'
            .'    :local ifaceRx 0; :local ifaceTx 0;'
            .'    :if ([:len $interface] > 0) do={'
            .'        :set ifaceRx [/interface get $interface rx-byte];'
            .'        :set ifaceTx [/interface get $interface tx-byte];'
            .'    };'
            .'    :if (($needBytesIn) && $ifaceRx > 0) do={:set webhookBytesIn $ifaceRx;};'
            .'    :if (($needBytesOut) && $ifaceTx > 0) do={:set webhookBytesOut $ifaceTx;};'
            .'}'
