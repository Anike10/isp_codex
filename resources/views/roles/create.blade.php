...
<div class="topbar">
    <div>
        <h1>Add Role</h1>
        <div class="muted">Create a role and choose permissions</div>
    </div>
    <a class="btn light" href="{{ route('roles.index') }}">Back</a>
</div>

<form method="post" action="{{ route('roles.store') }}" class="role-layout">
    @csrf
    <div class="card role-panel">
        ...
        <div class="grid" style="gap:14px">
            <div>
                ...
                <input name="name" value="{{ old('name') }}" placeholder="manager" required>
                <span class="muted">Use lowercase letters, numbers, dash, or underscore.</span>
            </div>
            <div>
                <label>Role Label</label>
                <input name="label" value="{{ old('label') }}" placeholder="Manager" required>
            </div>
