...
<table>
    <thead><tr><th>Date</th><th>Employee</th><th>Product</th><th>Source</th><th>Serials</th><th>Issued</th><th>Returned</th><th>Holding</th><th>Unit Value</th><th>Issued Value</th><th>Holding Value</th><th>Purpose</th><th>Approval</th><th>Action</th></tr></thead>
    <tbody>
    @forelse($assignments as $assignment)
        <tr data-href="{{ route('in-house-use.show', $assignment) }}"><td>{{ $assignment->assigned_at->format('d/m/Y') }}</td><td>{{ $assignment->employee->name }}</td><td>{{ $assignment->product->name }}<div class="muted">{{ $assignment->product->sku }}</div></td><td>{{ $assignment->source_condition === 'used' ? 'Used Stock' : 'New Stock' }}<div class="muted">{{ $assignment->warehouse->name }}</div></td><td>{{ $assignment->serial_numbers ?: ($assignment->serialless_quantity.' serial-less') }}</td><td>{{ $assignment->quantity }}</td><td>{{ $assignment->returnedQuantity() }}</td><td><span class="badge {{ $assignment->outstandingQuantity() > 0 ? 'pending' : '' }}">{{ $assignment->outstandingQuantity() }}</span></td><td>{{ number_format((float)$assignment->unit_price, 2) }}</td><td>{{ number_format((float)$assignment->total, 2) }}</td><td><strong>{{ number_format($assignment->outstandingValue(), 2) }}</strong></td><td>{{ $assignment->purpose ?? 'N/A' }}</td><td>@if($assignment->approval_document_path)<a class="btn light" target="_blank" rel="noopener" href="{{ route('in-house-use.approval-document', $assignment) }}">View</a>@else N/A @endif</td><td><a class="btn light" href="{{ route('in-house-use.show', $assignment) }}">View / Return</a></td></tr>
    @empty
        <tr><td colspan="14">No issue record found.</td></tr>
    @endforelse
    </tbody>
</table>
