...
<section class="card">
    ...
    <table>
        ...
        @forelse($employees->flatMap->assetAssignments->sortByDesc('assigned_at') as $assignment)
            <tr><td>{{ $assignment->employee?->name ?? $employees->firstWhere('id', $assignment->employee_id)?->name }}</td><td>{{ $assignment->assigned_at->format('Y-m-d') }}</td><td>{{ $assignment->product->name }}<div class="muted">{{ $assignment->source_condition === 'used' ? 'Used stock' : 'New stock' }}</div></td><td>{{ $assignment->serial_numbers ?: ($assignment->serialless_quantity.' serial-less') }}</td><td>{{ $assignment->quantity }}<div class="muted">Returned {{ $assignment->returnedQuantity() }} / Holding {{ $assignment->outstandingQuantity() }}</div></td><td>{{ number_format((float)$assignment->unit_price, 2) }}</td><td>{{ number_format((float)$assignment->total, 2) }}</td><td>{{ number_format($assignment->returnedValue(), 2) }}</td><td><strong>{{ number_format($assignment->outstandingValue(), 2) }}</strong></td><td>{{ $assignment->purpose ?? 'N/A' }}</td><td><a class="btn light" href="{{ route('in-house-use.show', $assignment) }}">View / Return</a></td></tr>
        @empty
