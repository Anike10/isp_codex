...
<form method="post" action="{{ route('products.store') }}" class="card form-grid">
    ...
    <div>
        ...
        <input name="category" value="{{ old('category') }}" list="categoryOptions" placeholder="Router, Cable, Computer Parts">
    </div>
    <div>
        <label>Sub Category</label>
        <input name="subcategory" value="{{ old('subcategory') }}" list="subcategoryOptions" placeholder="ONU, Patch Cord, RAM">
    </div>
