...
@forelse($vehicle->expenses as $expense)<tr role="link" tabindex="0" title="Click to view expense details" style="cursor:pointer" onclick="if (!event.target.closest('a,button')) window.location='{{ route('fleet.expenses.show', $expense) }}'" onkeydown="if (event.key === 'Enter') window.location='{{ route('fleet.expenses.show', $expense) }}'"><td>{{ $expense->expense_date->format('Y-m-d') }}</td><td><a href="{{ route('fleet.expenses.show', $expense) }}">{{ \App\Models\VehicleExpense::CATEGORIES[$expense->category] }}</a></td><td>{{ $expense->employee?->name ?? 'N/A' }}</td><td>{{ $expense->trip_reference ?? 'N/A' }}</td><td>{{ number_format((float)$expense->amount,2) }}</td><td>{{ $expense->description ?? 'N/A' }}</td></tr>@empty<tr><td colspan="6">No vehicle expenses.</td></tr>@endforelse</tbody></table></section>
@endsection
