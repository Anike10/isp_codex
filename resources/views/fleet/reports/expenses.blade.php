...
<section class="card" style="margin-bottom:16px"><h2>Vehicle-wise Total Expenses</h2><table><thead><tr><th>SL</th><th>Vehicle</th><th>Entries</th><th>Total</th></tr></thead><tbody>
@forelse($expenseByVehicle as $row)<tr role="link" tabindex="0" title="Click to view vehicle" style="cursor:pointer" onclick="if (!event.target.closest('a,button')) window.location='{{ route('fleet.show', $row->vehicle) }}'" onkeydown="if (event.key === 'Enter') window.location='{{ route('fleet.show', $row->vehicle) }}'"><td>{{ $loop->iteration }}</td><td><a href="{{ route('fleet.show',$row->vehicle) }}">{{ $row->vehicle->registration_no }} — {{ $row->vehicle->name }}</a></td><td>{{ $row->entry_count }}</td><td><strong>{{ number_format((float)$row->total_amount,2) }}</strong></td></tr>@empty<tr><td colspan="4">No expense data for the selected filters.</td></tr>@endforelse</tbody></table></section>

<section class="card"><h2>Itemized Expenses</h2>@include('partials.per_page')<table><thead><tr><th>SL</th><th>Date</th><th>Vehicle</th><th>Category</th><th>Driver</th><th>Trip</th><th>Qty</th><th>Amount</th><th>Entered By</th></tr></thead><tbody>
@forelse($expenseRows as $row)<tr role="link" tabindex="0" title="Click to view expense details" style="cursor:pointer" onclick="if (!event.target.closest('a,button')) window.location='{{ route('fleet.expenses.show', $row) }}'" onkeydown="if (event.key === 'Enter') window.location='{{ route('fleet.expenses.show', $row) }}'"><td>{{ $expenseRows->firstItem() + $loop->index }}</td><td>{{ $row->expense_date->format('Y-m-d') }}</td><td>{{ $row->vehicle->registration_no }}</td><td><a href="{{ route('fleet.expenses.show', $row) }}">{{ \App\Models\VehicleExpense::CATEGORIES[$row->category] }}</a></td><td>{{ $row->employee?->name ?? 'N/A' }}</td><td>{{ $row->trip_reference ?? 'N/A' }}</td><td>{{ $row->quantity ? rtrim(rtrim(number_format((float)$row->quantity,3,'.',''), '0'), '.').' '.($row->unit ?? '') : 'N/A' }}</td><td>{{ number_format((float)$row->amount,2) }}</td><td>{{ $row->creator?->name ?? 'N/A' }}</td></tr>@empty<tr><td colspan="9">No expenses found.</td></tr>@endforelse</tbody></table><div style="margin-top:14px">{{ $expenseRows->links() }}</div></section>
@endsection
