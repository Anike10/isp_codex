...
@forelse($maintenanceLogs as $log)<tr><td>{{ $log->service_date->format('Y-m-d') }}</td><td><a href="{{ route('fleet.show',$log->vehicle) }}">{{ $log->vehicle->registration_no }}</a></td><td>{{ $log->item?->name ?? $log->work_name ?? 'General' }}</td><td>{{ \App\Models\VehicleMaintenanceLog::ACTIONS[$log->action] ?? ucfirst($log->action) }}</td><td>{{ $log->mileage ? number_format($log->mileage) : 'N/A' }}</td><td>{{ number_format((float)$log->cost,2) }}</td><td>{{ $log->vendor ?? 'N/A' }}</td><td>{{ $log->details ?? 'N/A' }}</td><td><div class="actions">@foreach($log->photos as $index => $photo)<a href="{{ route('fleet.maintenance.photos.show', $photo) }}" target="_blank" rel="noopener">Photo {{ $index + 1 }}</a>@endforeach @if($log->youtube_url)<a href="{{ $log->youtube_url }}" target="_blank" rel="noopener">YouTube Video</a>@endif @if($log->photos->isEmpty() && !$log->youtube_url) N/A @endif</div></td><td>{{ $log->creator?->name ?? 'N/A' }}</td></tr>@empty<tr><td colspan="10">No maintenance logs found.</td></tr>@endforelse</tbody></table><div style="margin-top:14px">{{ $maintenanceLogs->links() }}</div></section>
@endsection
