...
@forelse($assignments as $assignment)<tr><td><a href="{{ route('fleet.show',$assignment->vehicle) }}">{{ $assignment->vehicle->registration_no }}</a></td><td>{{ \App\Models\VehicleAssignmentHistory::ROLES[$assignment->duty_role] }}</td><td>{{ $assignment->employee->name }}</td><td>{{ $assignment->start_date->format('d/m/Y') }}</td><td>{{ $assignment->end_date?->format('d/m/Y') ?? 'Current' }}</td><td><span class="badge {{ $assignment->end_date ? 'inactive' : 'active' }}">{{ $assignment->end_date ? 'Ended' : 'Current' }}</span></td><td>{{ $assignment->assigner?->name ?? 'N/A' }}</td><td>{{ $assignment->note ?? 'N/A' }}</td></tr>@empty<tr><td colspan="8">No duty history found.</td></tr>@endforelse</tbody></table><div style="margin-top:14px">{{ $assignments->links() }}</div></section>
@endsection
