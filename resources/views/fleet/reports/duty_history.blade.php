...
<section class="card">@include('partials.per_page')<table><thead><tr><th>SL</th><th>Vehicle</th><th>Role</th><th>Employee</th><th>Start</th><th>End</th><th>Status</th><th>Assigned By</th><th>Note</th></tr></thead><tbody>
@forelse($assignments as $assignment)<tr><td>{{ $assignments->firstItem() + $loop->index }}</td><td><a href="{{ route('fleet.show',$assignment->vehicle) }}">{{ $assignment->vehicle->registration_no }}</a></td><td>{{ \App\Models\VehicleAssignmentHistory::ROLES[$assignment->duty_role] }}</td><td>{{ $assignment->employee->name }}</td><td>{{ $assignment->start_date->format('Y-m-d') }}</td><td>{{ $assignment->end_date?->format('Y-m-d') ?? 'Current' }}</td><td><span class="badge {{ $assignment->end_date ? 'inactive' : 'active' }}">{{ $assignment->end_date ? 'Ended' : 'Current' }}</span></td><td>{{ $assignment->assigner?->name ?? 'N/A' }}</td><td>{{ $assignment->note ?? 'N/A' }}</td></tr>@empty<tr><td colspan="9">No duty history found.</td></tr>@endforelse</tbody></table><div style="margin-top:14px">{{ $assignments->links() }}</div></section>
@endsection
